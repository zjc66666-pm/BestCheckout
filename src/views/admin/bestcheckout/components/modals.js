...
import { getSetupReadiness } from '../readiness.js?rev=20260719-optional-growth-v116';
import { escapeHtml } from '../utils.js';

function funnelDisplayName(funnel, isZh) {
  if (funnel.systemCopy && funnel.systemCopy.name) {
    return isZh ? funnel.systemCopy.name.zh : funnel.systemCopy.name.en;
  }
  return funnel.name;
}
...
function createFunnelAudiencePresets(isZh) {
  return [
    ...
    },
    {
      ...
      title: isZh ? '客户条件（仅已识别客户）' : 'Customer conditions',
      copy: isZh ? '按客户标签、历史订单、累计消费或最近一次下单进行细分。' : 'Segment by customer tags, order history, lifetime spend, or the last order date.',
      rule: isZh ? '仅限已识别客户 ∧ 历史订单数 ≥ 1' : 'Recognized customer ∧ past orders ≥ 1',
      availability: isZh ? '仅适用于已识别客户' : 'Recognized customers only',
      restricted: true,
    },
    {
...
export function renderAddOfferModal(state, funnel, context) {
  ...
  const variantOptions = '<div class="product-choice-toolbar"><small>' + escapeHtml(copy('Product data updates automatically.', '商品数据会自动更新。')) + '</small><button type="button" class="button button-plain" data-action="refresh-product-data">' + icon('sync', 14) + '<span>' + escapeHtml(copy('Refresh product data', '刷新商品数据')) + '</span></button></div>' + availableVariants.map(function (variant) {
...
const AUDIENCE_FIELD_GROUPS = [
  {
    label: 'Customer conditions', labelZh: '客户条件（仅适用于已识别客户）', fields: [
...
function normaliseAudienceCondition(rule, isZh) {
  if (!rule || rule.field === 'logged_in' || rule.field === 'marketing_consent') return null;
  if (rule.field === 'customer_type') {
    const returning = String(rule.value || '').toLowerCase().includes('return');
    return {
      field: 'past_orders', operator: returning ? 'at_least' : 'equals', value: returning ? '1' : '0',
      fieldLabel: isZh ? '历史订单数' : 'Past orders', operatorLabel: returning ? (isZh ? '至少为' : 'is at least') : (isZh ? '等于' : 'equals'), shortLabel: isZh ? '订单' : 'Orders',
    };
  }
  return rule;
}

function audienceConditionsForModal(funnel, isZh) {
  const conditions = (Array.isArray(funnel.audienceConditions) ? funnel.audienceConditions : []).map(function (rule) { return normaliseAudienceCondition(rule, isZh); }).filter(Boolean);
  return conditions.length ? conditions : [{ field: 'cart_items', operator: 'at_least', value: '1' }];
}

function renderAudienceBuilder(funnel, isZh, products) {
  const funnelTitle = funnelDisplayName(funnel, isZh);
  const conditions = audienceConditionsForModal(funnel, isZh);
  const audienceRows = conditions.map(function (rule) { return audienceRuleRow(rule, isZh, products); }).join('');
  const template = audienceRuleRow({ field: 'cart_items', operator: 'at_least', value: '1' }, isZh, products);
  return '<section class="audience-builder"><header><span><strong>' + (isZh ? '购买流程条件（AND）' : 'Purchase flow conditions (AND)') + '</strong><small>' + (isZh ? '所有条件命中后，买家才会进入“' + escapeHtml(funnelTitle) + '”。' : 'Buyers enter “' + escapeHtml(funnelTitle) + '” only when every condition matches.') + '</small></span><em>' + (isZh ? '未命中 → Shopify Checkout' : 'No match → Shopify Checkout') + '</em></header><div class="audience-rule-list" data-audience-rule-list>' + audienceRows + '</div><template data-audience-rule-template>' + template + '</template><button type="button" class="audience-add-rule" data-action="add-audience-rule">' + icon('plus', 16) + '<span>' + (isZh ? '添加条件' : 'Add condition') + '</span></button></section>';
}
...
export function renderFunnelEntryModal(state, funnel) {
  ...
  const priorityFields = '<section class="funnel-priority-fields"><label>' + (isZh ? '漏斗优先级' : 'Funnel priority') + '<input type="number" name="priority" min="1" max="999" step="1" value="' + escapeHtml(priority) + '" required/><small>' + (isZh ? '数值越大优先级越高。多个漏斗同时命中时，只进入优先级最高的一个。' : 'Higher numbers win. If several Funnels match, buyers enter only the highest-priority one.') + '</small></label><label>' + (isZh ? '冲突处理' : 'Conflict handling') + '<input type="text" value="' + escapeHtml(isZh ? '首次命中（按优先级）' : 'First match by priority') + '" disabled/><small>' + (isZh ? '避免同一购物车同时进入多条路径。' : 'Prevents one cart from entering multiple journeys.') + '</small></label></section>';
  ...
  return modalShell('funnel-entry', isZh ? '配置漏斗入口' : 'Configure Funnel entry', funnelTitle, body, footer, true);
}
...
export function renderConnectProviderModal(state, selectedId, requestedStep) {
  ...
  let footer = '';
  if (step === 'select') {
    body = '<form id="provider-selection-form" class="form-stack">' + providerConnectStepper(step, isZh) + '<label>' + (isZh ? '支付服务商' : 'Payment provider') + '<select name="providerId">' + options + '</select><small>' + (isZh ? '选择接收 Checkout 款项的商户账户。下一步会显示适合该服务商的连接方式。' : 'Choose the merchant account that receives Checkout payments. The next step shows the best connection method for that provider.') + '</small></label><div class="secure-connect-summary"><span>' + icon('shield', 20) + '</span><div><strong>' + (isZh ? '优先使用安全授权' : 'Secure authorization when available') + '</strong><small>' + (isZh ? 'BestCheckout 只要求完成连接所必需的信息，并会在连接后自动完成支付状态更新。' : 'BestCheckout asks only for what is needed to connect the account and keeps payment status up to date automatically.') + '</small></div></div><details class="advanced-connection-note"><summary>' + (isZh ? '连接后会发生什么？' : 'What happens after I connect?') + '</summary><p>' + (isZh ? 'BestCheckout 会自动完成必要的技术配置，并检查账户是否可以用于 Checkout。你无需手动填写回调地址。' : 'BestCheckout completes the required technical setup automatically and checks that the account is ready for checkout. You do not need to enter callback addresses.') + '</p></details></form>';
    ...
    const checks = check(connection.providerId === selected && connection.authorizationState === 'verified', isZh ? '账户所有权已确认' : 'Merchant account ownership confirmed', isZh ? '已获得安全连接。' : 'A secure connection was received.') + check(passed, isZh ? '支付状态更新已设置' : 'Payment status updates are ready', isZh ? '支付、退款与失败状态会自动更新。' : 'Payments, refunds and failed payments will update automatically.') + check(passed, isZh ? '支付方式与安全测试已通过' : 'Payment methods and safe test passed', isZh ? '每笔订单仍会根据付款方式检查优惠是否可用。' : 'Each order will still check whether buyer offers are available.');
    body = (passed ? '<form id="provider-validation-form" class="form-stack"><input type="hidden" name="providerId" value="' + escapeHtml(selected) + '"/>' : '<div class="form-stack">') + providerConnectStepper(step, isZh) + '<div class="provider-connect-heading"><span class="provider-logo provider-' + escapeHtml(provider.id) + '">' + escapeHtml(provider.name.charAt(0)) + '</span><div><strong>' + escapeHtml(provider.name) + '</strong><small>' + (passed ? (isZh ? '账户连接检查已通过' : 'Account connection checks passed') : (isZh ? '准备检查账户连接' : 'Ready to check this account connection')) + '</small></div></div><ul class="provider-validation-list">' + checks + '</ul><div class="modal-callout"><span>' + icon('shield', 17) + '</span><span><strong>' + (passed ? (isZh ? '连接即将完成' : 'Ready to finish connection') : (isZh ? '校验尚未完成' : 'Validation has not run yet')) + '</strong><small>' + (passed ? (isZh ? '完成后，服务商卡片会显示已连接；凭证不会在 BestCheckout 中显示。' : 'After finishing, the provider card will show Connected. Credentials are never displayed in BestCheckout.') : (isZh ? '生产环境会等待账户授权、测试支付和支付状态处理均通过后再完成连接。' : 'Production waits for account authorization, a test payment and payment-status handling to pass.')) + '</small></span></div>' + (passed ? '</form>' : '</div>');
...
export function renderPublishModal(funnel, intent, state) {
  ...
  const rows = Object.keys(funnel.guardrails).filter(function (key) { return key !== 'fallback'; }).map(function (key) {
    const label = key === 'writeback' ? 'Order sync to Shopify' : key === 'shopifyAccess' ? 'Shopify store connection' : key === 'embed' ? 'Storefront connection' : key === 'graph' ? 'Checkout journey' : key === 'traffic' ? 'Traffic allocation' : key.charAt(0).toUpperCase() + key.slice(1);
  ...
  const store = state.store;
  const contract = '<dl class="detail-grid detail-grid-single deployment-contract"><div><dt>Checkout pages</dt><dd>Latest published pages are ready</dd></div><div><dt>Buyer audience</dt><dd>' + escapeHtml(funnel.audience || 'All eligible buyers') + '</dd></div><div><dt>Payment accounts</dt><dd>Ready for this checkout</dd></div><div><dt>Store data</dt><dd>Products, discounts and shipping rates are current</dd></div><div><dt>Order sync</dt><dd>Paid orders will continue to Shopify</dd></div></dl>';
  const body = '<div class="deployment-check-meta"><span>Checked ' + escapeHtml(check.checkedAt || 'Not evaluated') + '</span></div>' + contract + '<div class="publish-checks">' + rows + '</div>' + (blocked ? '<div class="modal-callout modal-callout-warning">' + icon('alert', 17) + '<span><strong>Publishing is blocked</strong><small>Complete the items marked as needing attention before sending buyers to this purchase flow. Tracking warnings never interrupt a completed payment.</small></span></div>' : '<div class="modal-callout">' + icon('shield', 17) + '<span><strong>Your live checkout stays stable</strong><small>BestCheckout saves this published version separately, so you can keep improving the next draft without changing buyers already in checkout.</small></span></div>');
  ...
  return modalShell('publish-funnel', 'Publish ' + funnel.name, 'BestCheckout checks your purchase flow before it goes live.', body, footer, false);
}

export function renderActivityDetailModal(event, locale) {
  const isZh = locale === 'zh';
  const text = function (en, zh) { return isZh ? zh : en; };
  const categories = {
    payment: text('Payment', '支付'),
    'post-purchase': text('Buyer offer', '买家优惠'),
    sync: text('Store data', '店铺数据'),
    writeback: text('Shopify order', 'Shopify 订单'),
    tracking: text('Sales tracking', '销售追踪'),
  };
  const title = isZh && event.titleZh ? event.titleZh : event.title;
  const detail = isZh && event.detailZh ? event.detailZh : event.detail;
  const impact = isZh && event.impactZh ? event.impactZh : event.impact;
  const observedAt = isZh && event.timeZh ? event.timeZh : event.time;
  const body = '<div class="activity-detail-head">' + badge(event.status) + '</div><p class="activity-detail-copy">' + escapeHtml(detail) + '</p><dl class="detail-grid detail-grid-single"><div><dt>' + escapeHtml(text('Update type', '更新类型')) + '</dt><dd>' + escapeHtml(categories[event.category] || event.category) + '</dd></div><div><dt>' + escapeHtml(text('Status', '状态')) + '</dt><dd>' + badge(event.status) + '</dd></div><div><dt>' + escapeHtml(text('Updated', '更新时间')) + '</dt><dd>' + escapeHtml(observedAt) + '</dd></div></dl><div class="modal-callout">' + icon('shield', 17) + '<span><strong>' + escapeHtml(text('What this means for your orders', '这对你的订单意味着什么')) + '</strong><small>' + escapeHtml(impact || text('Your checkout and orders are up to date.', '你的结账与订单状态均为最新。')) + '</small></span></div>';
  return modalShell('activity-detail', title, text('Update details', '更新详情'), body, button(text('Close', '关闭'), 'close-modal', { kind: 'primary' }), true);
}
...
export function renderInfoModal(kind) {
  const content = {
    ...
    capability: {
      title: 'How buyer offers stay safe',
      subtitle: 'BestCheckout checks whether an offer is available before a buyer sees it.',
      body: '<div class="capability-formula"><span>Payment provider</span><b>×</b><span>Payment method</span><b>×</b><span>Buyer market</span><b>×</b><span>Currency</span></div><ol class="modal-bullets"><li>BestCheckout checks the payment method, market and currency for each order.</li><li>When an additional offer is available, the buyer can accept it after checkout.</li><li>When it is not available, the buyer simply continues to the Thank you page with their original paid order.</li><li>If an order needs attention after payment, BestCheckout keeps it visible in Orders and continues syncing it to Shopify.</li></ol>',
    },
  }[kind];
