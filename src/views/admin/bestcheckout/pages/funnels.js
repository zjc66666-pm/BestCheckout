...
function renderGlobalFunnelRouter(state) {
  const routes = state.funnels.slice().sort(function (left, right) { return right.priority - left.priority; });
...
function renderFunnelList(state) {
  ...
  return '<div class="funnel-index-table" role="table"><div class="funnel-index-header" role="row"><span role="columnheader">' + escapeHtml(copy(state, 'Funnel', '漏斗')) + '</span><span role="columnheader">' + escapeHtml(copy(state, 'Status', '状态')) + '</span><span role="columnheader">' + escapeHtml(copy(state, 'Traffic', '流量')) + '</span><span role="columnheader">' + escapeHtml(copy(state, 'Conversion', '转化率')) + '</span><span role="columnheader">' + escapeHtml(copy(state, 'AOV', '客单价')) + '</span><span role="columnheader">' + escapeHtml(copy(state, 'Updated', '更新时间')) + '</span><span aria-hidden="true"></span></div>' + visible.map(function (funnel) {
    ...
    return '<div class="funnel-index-row" role="row"><span class="funnel-index-primary" role="cell"><button type="button" data-action="select-funnel" data-funnel-id="' + escapeHtml(funnel.id) + '"><strong data-i18n-skip>' + escapeHtml(funnelName(funnel, state)) + '</strong><em>' + escapeHtml(routeSummary) + '</em></button></span><span class="funnel-index-status" role="cell">' + badge(funnel.status) + '</span><span class="funnel-index-value" role="cell"><strong>' + escapeHtml(funnel.hostedTraffic + '%') + '</strong><small>BestCheckout</small></span><span class="funnel-index-value" role="cell"><strong>' + escapeHtml(funnel.conversion) + '</strong></span><span class="funnel-index-value" role="cell"><strong>' + escapeHtml(funnel.aov) + '</strong></span><span class="funnel-index-updated" role="cell">' + escapeHtml(updatedLabel) + '</span><span class="funnel-index-action" role="cell"><button type="button" data-action="select-funnel" data-funnel-id="' + escapeHtml(funnel.id) + '" aria-label="' + escapeHtml(openLabel + ' ' + funnelName(funnel, state)) + '"><span>' + escapeHtml(openLabel) + '</span>' + icon('chevron', 16) + '</button></span></div>';
  }).join('') + '</div>';
}
...
function renderFunnelEntryOverviewLegacy(funnel, state) {
  const fieldLabels = state.ui.locale === 'zh'
    ? { customer_type: '新客与老客', logged_in: '仅限已识别客户', country: '国家或地区', region: '州或省', city: '城市', customer_language: '客户语言', customer_tag: '客户标签', device: '设备', traffic_source: '流量来源', cart_total: '购物车金额', cart_contains: '购物车包含商品', cart_collection: '购物车包含系列', cart_sku: '购物车包含 SKU', past_orders: '历史订单数', lifetime_value: '客户累计消费', last_order: '最近一次下单' }
  ...
  const conditions = (funnel.audienceConditions || []).map(function (rule) {
    ...
    const value = rule.displayValue || (rule.field === 'logged_in' ? (state.ui.locale === 'zh' ? '已识别客户' : 'Recognized customer') : (rule.field === 'last_order' && /^[1-9]\d*$/.test(String(rule.value || '')) ? String(rule.value) + (state.ui.locale === 'zh' ? ' 天' : ' days') : (rule.value || '')));
...
function renderFunnelEntryOverview(funnel, state) {
  const fieldLabels = state.ui.locale === 'zh'
    ? { customer_type: '客户', logged_in: '仅限已识别客户', country: '国家/地区', region: '地区', city: '城市', customer_language: '语言', customer_tag: '客户标签', device: '设备', traffic_source: '来源', cart_total: '购物车', cart_contains: '购物车商品', cart_collection: '商品系列', cart_sku: 'SKU', past_orders: '历史订单', lifetime_value: '累计消费', last_order: '最近一次下单' }
    : { customer_type: 'Customer', logged_in: 'Account', country: 'Market', region: 'Region', city: 'City', customer_language: 'Language', customer_tag: 'Tag', device: 'Device', traffic_source: 'Source', cart_total: 'Cart', cart_contains: 'Cart', cart_collection: 'Collection', cart_sku: 'SKU', past_orders: 'Orders', lifetime_value: 'LTV', last_order: 'Last order' };
  const operatorLabels = state.ui.locale === 'zh'
    ...
    : { is: 'is', is_not: 'is not', one_of: 'is one of', not_one_of: 'is not one of', equals: 'equals', not_equal: 'does not equal', at_least: 'at least', at_most: 'at most', between: 'between', contains: 'contains', does_not_contain: 'does not contain', within_last: 'within', more_than_ago: 'before' };
  const conditions = (funnel.audienceConditions || []).map(function (rule) {
    if (rule.field === 'logged_in') {
      return '<span data-i18n-skip>' + escapeHtml(copy(state, 'Recognized customers only', '仅限已识别客户')) + '</span>';
    }
    const field = fieldLabels[rule.field] || rule.shortLabel || rule.fieldLabel || rule.field || '';
    const operator = operatorLabels[rule.operator] || rule.operatorLabel || rule.operator || '';
    const value = rule.displayValue || (rule.field === 'logged_in' ? (state.ui.locale === 'zh' ? '已识别客户' : 'Recognized customer') : (rule.field === 'last_order' && /^[1-9]\d*$/.test(String(rule.value || '')) ? String(rule.value) + (state.ui.locale === 'zh' ? ' 天' : ' days') : (rule.value || '')));
...
function renderDetailHeader(funnel, state) {
  ...
  return pageHeader(funnelName(funnel, state), '', actions, [{ label: copy(state, 'Funnels', '漏斗'), route: 'funnels' }, { label: funnelName(funnel, state) }]);
}
...
function renderMoreSettings(funnel, state) {
  const guardrails = Object.keys(funnel.guardrails).filter(function (key) { return key !== 'fallback'; }).map(function (key) {
    const label = key === 'writeback' ? copy(state, 'Order sync to Shopify', '订单同步至 Shopify') : key === 'embed' ? copy(state, 'Storefront connection', '店铺前台连接') : key === 'shopifyAccess' ? copy(state, 'Shopify store connection', 'Shopify 店铺连接') : key === 'graph' ? copy(state, 'Buyer journey', '购买路径') : key.charAt(0).toUpperCase() + key.slice(1);
...
function renderFunnelDetail(funnel, state) {
  return '<div class="page-stack funnel-detail-page">' + renderDetailHeader(funnel, state) + renderFunnelEntryOverview(funnel, state) + '<section class="card funnel-canvas-card">' + renderCanvas(funnel, state) + renderInspector(funnel, state) + '</section></div>';
}
