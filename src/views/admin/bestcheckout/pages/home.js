...
function renderPrelaunchOverviewLegacy(state) {
  ...
  const healthNoteCopy = isZh ? '结账域名、支付、转化追踪、店铺前台连接和订单同步会显示为已就绪或需要处理。' : 'Checkout domain, payments, sales tracking, storefront connection and order sync will surface as healthy or needing action.';
...
function renderLaunchChecklist(state) {
  const isZh = state.ui.locale === 'zh';
  const readiness = getSetupReadiness(state);
  if (readiness.live) return '';
  const items = readiness.checks;
  const next = items.find((item) => item.state !== 'complete') || readiness.launch;
  const unresolvedMinutes = items.filter((item) => item.state !== 'complete').reduce((sum, item) => sum + item.minutes, 0);
  const connectedCount = readiness.completeCount + 1;
  const totalSteps = items.length + 1;
  const completionPercent = Math.round((connectedCount / totalSteps) * 100);
  const connectionRow = '<article class="launch-checklist-row is-complete" role="listitem"><span class="launch-checklist-icon">' + icon('check', 14) + '</span><div><strong>' + escapeHtml(isZh ? 'Shopify 店铺已连接' : 'Shopify store connected') + '</strong><small>' + escapeHtml(isZh ? '你的店铺已连接，商品、优惠和订单信息会保持同步。' : 'Your store is connected and its product, discount, and order data stays in sync.') + '</small></div><span class="launch-checklist-status">' + escapeHtml(isZh ? '已完成' : 'Complete') + '</span></article>';
  const rows = items.map(function (item) {
    const isComplete = item.state === 'complete';
    const isCurrent = !isComplete && item.id === next.id;
    const rowClass = isComplete ? ' is-complete' : isCurrent ? ' is-current' : ' is-upcoming';
    const copy = isComplete
      ? (isZh ? '已完成，可随时查看或修改。' : 'Complete. You can review it anytime.')
      : isCurrent
        ? (isZh ? item.detailZh : item.detail)
        : (isZh ? '完成当前任务后继续。' : 'Continue here after the current task is complete.');
    const action = isCurrent
      ? '<button type="button" class="launch-checklist-action" data-action="onboarding-open-step" data-target-route="' + escapeHtml(item.route) + '">' + escapeHtml(setupActionLabel(state, item)) + icon('chevron', 14) + '</button>'
      : isComplete
        ? '<span class="launch-checklist-status">' + escapeHtml(isZh ? '已完成' : 'Complete') + '</span>'
        : '<span class="launch-checklist-status">' + escapeHtml(isZh ? '下一步' : 'Up next') + '</span>';
    return '<article class="launch-checklist-row' + rowClass + '" role="listitem"><span class="launch-checklist-icon">' + (isComplete ? icon('check', 14) : isCurrent ? icon('arrow', 14) : '') + '</span><div><strong>' + escapeHtml(isZh ? item.titleZh : item.title) + '</strong><small>' + escapeHtml(copy) + '</small></div>' + action + '</article>';
  }).join('');
  const publishRow = readiness.requiredReady
    ? '<article class="launch-checklist-row is-current launch-checklist-publish" role="listitem"><span class="launch-checklist-icon">' + icon('play', 14) + '</span><div><strong>' + escapeHtml(isZh ? readiness.launch.titleZh : readiness.launch.title) + '</strong><small>' + escapeHtml(isZh ? readiness.launch.detailZh : readiness.launch.detail) + '</small></div><button type="button" class="launch-checklist-action" data-action="onboarding-open-step" data-target-route="' + escapeHtml(readiness.launch.route) + '">' + escapeHtml(setupActionLabel(state, readiness.launch)) + icon('chevron', 14) + '</button></article>'
    : '';
  const releaseState = readiness.requiredReady
    ? (isZh ? '所有必需任务已完成，可以预览并发布。' : 'Everything required is ready. Preview and publish when you are ready.')
    : (isZh ? '完成上方任务后，即可预览并发布。' : 'Complete the tasks above, then preview and publish.');
  const optionalGrowth = '<aside class="launch-checklist-optional"><span>' + icon('sparkles', 16) + '</span><div><strong>' + escapeHtml(isZh ? '添加 Upsell / Downsell（可选）' : 'Add Upsell / Downsell (optional)') + '</strong><small>' + escapeHtml(isZh ? '不会影响上线；基础购买流程跑通后，可随时添加来提升客单价。' : 'It never delays launch. Add it anytime after your purchase flow is running to lift order value.') + '</small></div><button type="button" data-action="onboarding-open-step" data-target-route="funnels">' + escapeHtml(isZh ? '添加' : 'Add') + icon('chevron', 14) + '</button></aside>';
  return '<section class="launch-checklist"><header class="launch-checklist-head"><div><span class="quickstart-kicker">' + escapeHtml(isZh ? '开始设置' : 'Getting started') + '</span><h2>' + escapeHtml(isZh ? '完成首个购买流程的上线准备' : 'Get your first purchase flow ready to launch') + '</h2><p>' + escapeHtml(isZh ? '每次只处理一个当前任务，其余步骤会在完成后接续显示。' : 'Focus on one task at a time. The next step is ready when you finish.') + '</p></div><div class="launch-checklist-progress"><div><strong>' + connectedCount + ' / ' + totalSteps + '</strong><span>' + escapeHtml(isZh ? '已完成' : 'completed') + '</span></div><div role="progressbar" aria-label="' + escapeHtml(isZh ? '上线准备进度' : 'Launch setup progress') + '" aria-valuemin="0" aria-valuemax="100" aria-valuenow="' + completionPercent + '"><i style="width:' + completionPercent + '%"></i></div><small>' + escapeHtml(isZh ? '预计还需 ' + unresolvedMinutes + ' 分钟' : '~' + unresolvedMinutes + ' min remaining') + '</small></div></header><div class="launch-checklist-rows" role="list">' + connectionRow + rows + publishRow + '</div><footer class="launch-checklist-footer"><span>' + icon(readiness.requiredReady ? 'check' : 'shield', 16) + '</span><p><strong>' + escapeHtml(isZh ? '准备发布' : 'Ready to publish') + '</strong><small>' + escapeHtml(releaseState) + '</small></p></footer>' + optionalGrowth + '</section>';
}
...
export function renderHome(state) {
  ...
  const activity = '<section class="card"><div class="card-pad">' + sectionHeader('Recent activity', 'Payments, buyer offers, store updates and order sync in one timeline.', routeButton('View all', 'activity', { kind: 'plain' })) + '<ul class="activity-compact-list">' + recent + '</ul></div></section>';
  ...
  return '<div class="page-stack">' + header + renderLaunchChecklist(state) + liveBanner + dashboard + '</div>';
}
