...
function brand() {
  return '<a class="help-brand" href="#/landing" aria-label="BestCheckout home"><img class="portal-mark" src="../assets/brand-mark.svg?v=20260723brandmark1" alt="" aria-hidden="true"/><strong>BestCheckout</strong></a>';
}
...
function setupArticle() {
  return '<article class="help-article">'
    ...
    + '<p class="help-article-lead">Create and release a Shopify Dev Dashboard app, copy its Client ID and Secret, then enter those values in BestCheckout.</p>'
    + '<div class="help-callout">' + icon('shield', 19) + '<div><strong>You only need three values at the end</strong><p>Your <b>myshopify.com</b> store address, Client ID, and Secret. Keep the app in your Shopify organization; BestCheckout never takes ownership of it.</p></div></div>'
    ...
    + '<section><h2>4. Copy the Client ID and Secret</h2><p>Open <b>Apps</b>, select your app, then open <b>Settings</b>. In <b>Credentials</b>, copy the Client ID and Secret with Shopify’s copy buttons.</p><figure class="help-screenshot"><img src="../help/assets/shopify-dev-dashboard-credentials-redacted.png" alt="Shopify Dev Dashboard Credentials card with all account and credential values redacted" /><figcaption>Apps &rarr; your app &rarr; Settings &rarr; Credentials. The example is cropped and all account, email, Client ID, and Secret values are redacted.</figcaption></figure><p class="help-muted">Treat the Secret as a password. Do not put it in chat, email, spreadsheets, or source code. If it is exposed, use Shopify’s <b>Rotate</b> action immediately and replace the value in BestCheckout.</p></section>'
    + '<section><h2>5. Connect in BestCheckout</h2><p>Return to BestCheckout and enter <b>your-store.myshopify.com</b>, the Client ID, and the Secret. Select <b>Connect store</b>. BestCheckout verifies the connection before any checkout traffic can go live.</p></section>'
    + '<section><h2>Official Shopify references</h2><ul><li><a href="https://shopify.dev/docs/apps/build/dev-dashboard/create-apps-using-dev-dashboard" target="_blank" rel="noopener">Create apps using Dev Dashboard</a></li><li><a href="https://shopify.dev/docs/apps/build/dev-dashboard/get-api-access-tokens?lang=node" target="_blank" rel="noopener">Find Client ID and Secret</a></li><li><a href="https://shopify.dev/docs/apps/build/authentication-authorization/client-secrets" target="_blank" rel="noopener">Keep client credentials secure</a></li></ul></section>'
