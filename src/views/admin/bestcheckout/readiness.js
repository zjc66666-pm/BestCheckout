...
export function getSetupReadiness(state) {
  ...
  const checks = [
    ...
    check('tracking', 'settings?tab=tracking', 1, 'Set conversion tracking', '设置订单转化追踪', trackingReady, 'Your order conversions are being recorded normally.', '订单转化已开始正常记录。', 'Connect your advertising or analytics tools and confirm order conversions can be recorded.', '连接广告或数据分析工具，确认订单转化可以正常记录。', 'Tracking health monitor', '转化追踪状态'),
  ];
  const launch = {
