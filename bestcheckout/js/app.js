...
(function () {
  ...
  var FLOW_LIST_SEED = [
    { id: 'first-order-boost', name: 'First-order boost', description: 'Give new customers a relevant one-click Upsell after checkout.', audience: 'First-time shoppers', entry: 'New customers', priority: 10, status: 'Live', traffic: '45%', conversion: '5.2%', aov: '$62.40', updated: 'Today, 10:42', summary: '1 checkout · 1 upsell · 1 Thank you page' },
    { id: 'returning-customer-offer', name: 'Returning customer offer', description: 'Show returning customers a Downsell after they decline the Upsell.', audience: 'Returning shoppers', entry: 'Returning customers', priority: 20, status: 'Draft', traffic: '0%', conversion: '—', aov: '—', updated: 'Saved today', summary: '1 checkout · 1 upsell · 1 downsell · 1 Thank you page' },
    { id: 'smooth-checkout', name: 'Smooth checkout', description: 'A focused checkout for all other shoppers, followed by the Thank you page.', audience: 'All remaining shoppers', entry: 'All other shoppers', priority: 30, status: 'Live', traffic: '55%', conversion: '4.1%', aov: '$54.80', updated: 'Yesterday', summary: '1 checkout · No Upsell or Downsell · 1 Thank you page', isDefault: true },
    { id: 'seasonal-returning-offer', name: 'Seasonal returning-customer offer', description: 'A limited-time Upsell kept ready for a future returning-customer campaign.', audience: 'Returning shoppers', entry: 'Returning customers', priority: 15, status: 'Paused', traffic: '0%', conversion: '—', aov: '—', updated: 'Paused today', summary: '1 checkout · 1 upsell · 1 Thank you page', mock: true }
  ];
  ...
  function flowPrimaryAction(flow) {
    if (flowIsLive(flow)) return { id: 'pause', nextStatus: 'Paused', label: 'Pause purchase flow', notice: 'Purchase flow paused. Shoppers will continue matching the next live purchase flow or Shopify Checkout.' };
  ...
  var FLOW_PURPOSES = {
    ...
    'smooth-checkout': 'A focused checkout for all other shoppers, followed by the Thank you page.'
  };
  function flowPurpose(flow) {
    return flow.description || FLOW_PURPOSES[flow.id] || 'Arrange the pages shoppers see in this purchase flow.';
  }
  ...
  function flowEntrySummary(flow) {
    ...
    var defaultEntries = ['All eligible shoppers', 'All remaining shoppers', 'All other shoppers', ''];
    if (defaultEntries.indexOf(flow.entry || '') !== -1) {
      ...
      return hasHigherPriorityFlow ? t('Shoppers not matched by higher-priority purchase flows') : t('All shoppers');
    }
    return t(flow.entry);
  }
  ...
  function openCreateFlow() {
    var goals = [
      ...
      { id: 'completion', label: 'Improve checkout completion', desc: 'Start with a focused Checkout and add Upsells or Downsells later.', flowDescription: 'A focused checkout for all other shoppers, followed by the Thank you page.', path: 'Checkout → Thank you', name: 'Smooth checkout', summary: '1 checkout · No Upsell or Downsell · 1 Thank you page' }
    ];
    ...
    var render = function () {
      ...
      var body = '<div class="fc-create-lead">' + t('What would you like this purchase flow to improve?') + '</div><div class="fc-create-goals">' + goals.map(function (goal) {
        ...
        }).join('') + '</div><div class="fc-create-priority"><label>' + t('Priority') + '<input id="flow-new-priority" class="input" type="number" min="1" step="10" value="' + esc(nextPriority) + '"></label><p>' + t('Higher numbers are checked first. You can change this later.') + '</p></div><div class="fc-create-safe"><span class="fc-create-safe-icon" aria-hidden="true">i</span><div><b>' + t('Nothing goes live when you create it') + '</b><p>' + t('A draft is created with Shopify Checkout as the safety route. Set the shopper entry from the flow after creating it.') + '</p></div></div>';
      ...
      modal.querySelector('[data-flow-next]').onclick = function () {
        ...
        flows.push({ id: id, name: selected.name, description: selected.flowDescription, audience: 'All remaining shoppers', entry: 'All remaining shoppers', priority: priority, status: 'Draft', traffic: '0%', conversion: '—', aov: '—', updated: 'Just now', summary: selected.summary });
  ...
  function openCreateFlowWizard() {
    ...
    var render = function () {
      ...
      if (step === 'goal') {
        ...
        var conditions = entryConditions.length ? entryConditions.map(function (condition, index) { return conditionRow(index, condition); }).join('') : '<div class="fcw-condition-empty">' + t('No conditions added. All eligible shoppers can enter this purchase flow.') + '</div>';
      ...
      modal.querySelector('[data-fcw-next]').onclick = function () {
        ...
        var audience = entryConditions.length ? 'Custom audience' : 'All eligible shoppers';
  ...
  function fnRuleSummary(edge) {
    ...
    return label ? t(label) : t('All eligible shoppers');
  }
  ...
  function flowJourneyStage(st, type, options) {
    ...
    return '<section class="fj-stage fj-stage-' + esc(type) + '"><header><div>' + eyebrow + '<h3>' + t(copy.title || fnStageName(type)) + '</h3><p>' + t(copy.description || '') + '</p></div></header><div class="fj-stage-nodes">' + (copy.beforeNodes || '') + (cards || empty) + '</div></section>';
  }
  function flowTrafficCard(st) {
    ...
    return '<section class="fj-traffic panel card-pad"><div class="fj-card-head"><div><span class="fj-eyebrow">' + t('Traffic allocation') + '</span><h2>' + t('How entered shoppers are distributed') + '</h2><p>' + t('Split entered shoppers between BestCheckout Checkout pages.') + '</p></div><button type="button" class="btn btn-default" data-flow-traffic>' + t('Configure traffic') + '</button></div><div class="fj-traffic-list">' + rows + '</div></section>';
  }
  ...
  function flowJourneyStoreStage(flow, entryText) {
    return '<section class="fj-stage fj-stage-store"><header><div><h3>' + t('Shopify store') + '</h3><p>' + t('Matching shoppers begin here.') + '</p></div></header><div class="fj-stage-nodes"><article class="fj-node fj-node-store"><div class="fj-node-head"><span class="fj-stage-icon">S</span><div><strong>lavender-labs.myshopify.com</strong><span class="fj-node-meta">' + esc(entryText) + '</span></div></div><button type="button" class="fj-store-link" data-flow-entry>' + t('Configure entry') + '</button></article></div><div class="fj-stage-hint">' + t('Matching shoppers enter the BestCheckout journey.') + '</div></section>';
  }
  function flowJourneyFallback() {
    return '<div class="fj-fallback"><span class="fj-fallback-icon">S</span><div><span class="fj-eyebrow">' + t('System fallback') + '</span><strong>' + t('Unmatched shoppers continue to Shopify Checkout.') + '</strong><p>' + t('Always enabled. This route cannot be edited or removed.') + '</p></div></div>';
  }
  ...
  function renderFlowJourney(flow, st, dirty) {
    ...
    var journeyStages = [
      ...
      flowJourneyStage(st, 'checkout', { title: 'Checkout', description: 'Collect payment and confirm the order.', icon: '1', beforeNodes: flowJourneyTrafficMini(st) })
    ];
  ...
  function openFlowEntryEditorLegacy(flow) {
    ...
    function render() {
      var conditions = buf.length ? buf.map(row).join('') : '<div class="fe-empty">' + t('No user attributes added. All shoppers can enter this flow.') + '</div>';
      modal.innerHTML = XSTYLE + FSTYLE + '<style>' +
        ...
        '</style><div class="xp-mc fe-modal"><div class="xp-mh">' + t('Configure shopper entry') + '</div><div class="xp-mb"><p class="fe-intro"><strong>' + t('Who can enter this purchase flow?') + '</strong><br>' + t('Add cart, storefront, or customer conditions. Every condition must match before a shopper enters this flow.') + '</p><div class="fe-list" data-entry-list>' + conditions + '</div><button type="button" class="btn btn-default fe-add" data-entry-add>+ ' + t('Add condition') + '</button><div class="fe-recognized">' + t('Customer attributes apply only to recognized Shopify customers. Shoppers who are not recognized or do not match continue to Shopify Checkout.') + '</div><p class="fe-msg" data-entry-msg></p></div><div class="xp-mf"><button type="button" class="btn btn-default" data-entry-cancel>' + t('Cancel') + '</button><button type="button" class="btn btn-primary" data-entry-save>' + t('Apply') + '</button></div></div>';
      bcI18n(modal);
      ...
      modal.querySelector('[data-entry-save]').onclick = function () {
        ...
        bcFlowUpdate(flow.id, { entryConditions: buf, entry: buf.length ? 'Custom audience' : 'All eligible shoppers', audience: buf.length ? 'Custom audience' : 'All eligible shoppers', updated: t('Just now') });
        close(); toast(t('Shopper entry saved')); renderFunnel(flow.id);
      };
    }
    document.body.appendChild(modal); modal.addEventListener('click', function (event) { if (event.target === modal) close(); }); render();
  }
  function openFlowEntryEditor(flow) {
    ...
    function render() {
      var conditions = buf.length ? buf.map(function (condition, index) { return row(index, condition); }).join('') : '<div class="fe-empty">' + t('No conditions added. All eligible shoppers can enter this purchase flow.') + '</div>';
      modal.innerHTML = XSTYLE + FSTYLE + '<style>' +
        ...
        '</style><div class="xp-mc fe-modal"><div class="xp-mh"><div class="fe-modal-title"><strong>' + t('Configure shopper entry') + '</strong><small>' + esc(t(flow.name)) + '</small></div><button type="button" class="fe-close" data-entry-cancel aria-label="' + t('Close') + '">×</button></div><div class="xp-mb"><section class="fe-layer"><span class="fe-layer-icon">1</span><div><b>' + t('Layer 1: Purchase flow routing') + '</b><strong>' + t('Choose the purchase flow first, then its Checkout page.') + '</strong><p>' + t('Shoppers who do not match this flow continue to Shopify Checkout.') + '</p></div></section><section class="fe-routing"><div class="fe-control"><label>' + t('Purchase flow priority') + '</label><input class="input" type="number" min="1" step="1" data-entry-priority value="' + esc(flow.priority) + '"></div><div class="fe-control"><label>' + t('Conflict handling') + '</label><div class="fe-readonly">' + t('Highest priority match') + '</div></div><p class="fe-routing-hint">' + t('Higher values win. If several purchase flows match, shoppers enter only the highest-priority flow.') + '</p></section><section class="fe-conditions"><header class="fe-conditions-head"><div><strong>' + t('Who enters this purchase flow? (AND)') + '</strong><p>' + t('A shopper enters only when every condition below matches.') + '</p></div><span class="fe-fallback">' + t('No match → Shopify Checkout') + '</span></header><div class="fe-list" data-entry-list>' + conditions + '</div><button type="button" class="fe-add" data-entry-add>+ ' + t('Add condition') + '</button><p class="fe-helper">' + t('Combine customer identity, storefront context, and cart conditions. Customer order and tag conditions apply only to signed-in shoppers.') + '</p></section><p class="fe-msg" data-entry-msg></p></div><div class="xp-mf"><button type="button" class="btn btn-default" data-entry-cancel>' + t('Cancel') + '</button><button type="button" class="btn btn-primary" data-entry-save>' + t('Save shopper entry') + '</button></div></div>';
      bcI18n(modal);
      ...
      modal.querySelector('[data-entry-save]').onclick = function () {
        ...
        bcFlowUpdate(flow.id, { entryConditions: buf, entry: buf.length ? 'Custom audience' : 'All eligible shoppers', audience: buf.length ? 'Custom audience' : 'All eligible shoppers', priority: priority, updated: t('Just now') });
        close(); toast(t('Shopper entry saved')); renderFunnel(flow.id);
      };
    }
    document.body.appendChild(modal); modal.addEventListener('click', function (event) { if (event.target === modal) close(); }); render();
    ...
    var node = function (nd) {
      ...
      if (fnIsSource(nd.type)) {
        return '<div class="fc-node fc-src t-shopify" data-id="' + nd.id + '" style="left:' + pos.x + 'px;top:' + pos.y + 'px">' +
          ...
          '<div class="fc-node-body"><div class="fc-src-dom">lavender-labs.myshopify.com</div>' +
            '<div class="fc-src-tag">' + t('Traffic source — shoppers enter the funnel here') + '</div>' +
            '<div class="fc-src-entry"><small>' + t('Shopper entry') + '</small><b>' + esc(flowEntrySummary(flow)) + '</b></div>' +
...
'<button class="btn btn-default" data-z="out" title="Zoom out">−</button><span class="fc-zval" id="fc-z">100%</span><button class="btn btn-default" data-z="in" title="Zoom in">+</button><button class="btn btn-default" data-z="fit">' + t('Fit') + '</button>' +
  ...
  function openCheckoutTrafficEditor(fromId, s0, buf) {
    ...
    m.innerHTML = XSTYLE + FSTYLE + '<style>' +
      ...
      '</style><div class="xp-mc ct-mc"><div class="ct-head"><h2>' + t('Set Checkout traffic rules') + '</h2><p>' + t('First decide who enters this Purchase flow, then decide which Checkout those shoppers use.') + '</p><button type="button" class="ct-close" data-ct-close aria-label="' + t('Close') + '">×</button></div>' +
        '<div class="ct-body"><div class="ct-audience"><span class="ct-audience-icon">◉</span><div class="ct-audience-copy"><small>' + t('Applies to') + '</small><strong>' + t('Shoppers who entered this purchase flow') + '</strong><span>' + esc(flowEntrySummary(flow)) + '</span></div><button type="button" class="ct-entry" data-ct-entry>' + t('Edit entry') + '</button></div>' +
          ...
          '<div class="ct-separate"><span class="ct-separate-icon">⌘</span><div><strong>' + t('Use a separate Purchase flow for a different audience') + '</strong><p>' + t('Create a separate flow when shoppers should follow a different journey. Use this split only for a default route or an A/B experiment within the same audience.') + '</p></div></div>' +
  ...
  function openRuleEditor(fromId) {
    ...
    var m = document.createElement('div'); m.className = 'xp-modal';
    var entryNote = isEntryRouting ? '<div class="rb-modal-note">' + t('Shopper eligibility is set in Shopper entry. This screen only allocates shoppers who already entered the flow.') + '</div>' : '';
    ...
    function render() {
      ...
      buf.forEach(function (br, bi) {
        ...
        html += '<div class="rb-branch ' + (fb ? 'fallback' : '') + '" data-bi="' + bi + '">' +
          ...
          '</div>' +
          routingHtml +
          '<div class="rb-section-l">' + t(isEntryRouting ? 'Traffic share of entered shoppers:' : 'Traffic share among the eligible:') + '</div>' +
          weightHtml +
        '</div>';
      });
  ...
  function offerProductTriggerHtml(product) {
    ...
    return '<span class="fo-product-avatar">' + esc((product.name || '?').charAt(0).toUpperCase()) + '</span><span class="fo-product-copy"><strong>' + esc(product.name) + '</strong><small>$' + esc(product.price) + ' <s>$' + esc(product.compareAt) + '</s></small></span><span class="fo-product-caret">⌄</span>';
  }
  ...
  function openPagePicker(opts) {
    ...
    var fromRow = mode === 'add' && !opts.offerAdd ? renderFromRow() : '';
    ...
    m.querySelector('#pp-ok').onclick = function () {
      ...
      else { close(); if (opts.offerAdd) fnJourneyAddOffer(selType, selTpl); else fnAddPage(selType, selTpl, selParent || ''); }
    };
  }
  ...
  const FLOW_SYSTEM_STYLE = '<style>' +
    ...
    '.fj-summary{margin:0 0 16px}.fj-summary-grid{display:grid;grid-template-columns:2fr 1fr 1.45fr;gap:0;margin-top:16px;border-top:1px solid var(--hair)}.fj-summary-grid>div{padding:13px 16px 2px 0;display:flex;flex-direction:column;gap:4px}.fj-summary-grid>div+div{border-left:1px solid var(--hair);padding-left:16px}.fj-summary-grid small{font-size:11.5px;color:var(--ink-muted)}.fj-summary-grid strong{font-size:13.5px;color:var(--ink)}' +
