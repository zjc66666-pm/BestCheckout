...
(function () {
  ...
  function bcEditHash(k, tplId, returnTo) {
    var b = '#/online-store/edit/aura/' + bcPage(k) + '?from=bestcheckout';
    if (returnTo) b += '&return=' + encodeURIComponent(returnTo);
  ...
  function bcTplThumb(tp, kind) {
    if (kind) {
      var preview = kind === 'checkout'
        ? '<div class="pl-preview pl-preview-checkout"><div class="pl-preview-nav"><b>' + t('Checkout') + '</b><i></i></div><div class="pl-preview-checkout-body"><div class="pl-preview-fields"><div class="pl-preview-form-stack"><span></span><span></span><span></span></div><div class="pl-preview-payment-block"><i></i><div class="pl-preview-card-fields"><span></span><span></span><span></span></div></div><em class="pl-preview-pay"></em></div><aside><i class="pl-preview-summary-title"></i><p><i></i><b></b></p><p><i></i><b></b></p><span class="pl-preview-summary-divider"></span><div class="pl-preview-summary-discount"><i></i><span></span></div><div class="pl-preview-summary-totals"><p><b></b><b></b></p><p class="pl-preview-summary-grandtotal"><b></b><b></b></p></div></aside></div></div>'
        : kind === 'thankyou'
  ...
  function openPageCreator() {
    ...
    var modal = bcModal(t('Create page'),
      ...
      t('Create and edit'), function () {
        ...
        location.hash = bcEditHash(type, created.id, '#/pages');
      });
  ...
  function renderTemplates(filter) {
    ...
    var cards = pages.map(function (item) {
      ...
      var cardActions = tp.soon
        ? '<span class="pl-coming-soon">' + t('Coming soon') + '</span>'
        : '<div class="pl-card-actions"><a class="btn btn-primary" href="' + bcEditHash(pt.key, tp.id, '#/pages') + '">' + t('Edit') + '</a><details class="pl-more"><summary aria-label="More page actions">•••</summary><div class="pl-more-menu"><button type="button" data-page-preview="' + pt.key + ':' + tp.id + '">' + t('Preview') + '</button><button type="button" data-page-duplicate="' + pt.key + ':' + tp.id + '">' + t('Duplicate') + '</button></div></details></div>';
      var previewTrigger = tp.soon ? '' : '<button type="button" class="pl-preview-open" data-page-preview="' + pt.key + ':' + tp.id + '">' + t('Preview') + '</button>';
      return '<article class="pl-card' + (tp.soon ? ' is-soon' : '') + '"><div class="pl-visual">' + bcTplThumb(tp, pt.key) + previewTrigger + '</div>' +
        '<div class="pl-card-body"><div class="pl-card-top"><span class="pl-type pl-type-' + pt.key + '">' + t(pt.label) + '</span><span class="pl-status' + stateClass + '"><i></i>' + t(state) + '</span></div>' +
        '<h2>' + esc(t(tp.name)) + '</h2>' +
        '<div class="pl-usage">' + t('Used in') + ' <b>' + usageText + '</b></div>' + cardActions + '</div></article>';
    }).join('');
    root.innerHTML = wrap(GSTYLE + FSTYLE + '<style>' +
      ...
      '.pl-page .pl-grid{grid-template-columns:repeat(4,minmax(0,1fr));gap:14px}.pl-page .pl-card{border-radius:13px;box-shadow:0 1px 3px rgb(16 24 40 / 5%);transition:transform .16s ease,box-shadow .16s ease,border-color .16s ease}.pl-page .pl-card:hover{border-color:#bfd2ea;box-shadow:0 10px 24px rgb(29 72 120 / 12%);transform:translateY(-2px)}.pl-page .pl-card .cg-thumb{height:138px;padding:9px;background:linear-gradient(180deg,#f8fafc 0%,#f1f4f8 100%);border-bottom-color:#e5eaf0}.pl-page .pl-preview{border-radius:8px;border-color:#e2e8f0;box-shadow:0 3px 10px rgb(16 24 40 / 7%)}.pl-page .pl-preview-nav{height:21px;padding:0 8px}.pl-page .pl-preview-checkout-body{gap:6px;padding:7px}.pl-page .pl-preview-offer{gap:9px;padding:10px 12%}.pl-page .pl-preview-product{width:43px;height:43px;border-radius:8px;font-size:18px}.pl-page .pl-card-body{padding:12px;gap:7px}.pl-page .pl-type{font-size:10px;letter-spacing:.55px}.pl-page .pl-status{padding:3px 6px;font-size:10.5px}.pl-page .pl-card h2{font-size:14px;line-height:1.25}.pl-page .pl-description{min-height:31px;font-size:12px;line-height:1.4}.pl-page .pl-facts{gap:7px 8px;padding-top:1px}.pl-page .pl-facts small{font-size:10.5px}.pl-page .pl-facts b{font-size:11.5px}.pl-page .pl-metric{margin-top:0;padding:8px}.pl-page .pl-metric strong{font-size:16px}.pl-page .pl-actions{gap:6px;padding-top:2px}.pl-page .pl-actions .btn{min-width:0;padding-left:7px;padding-right:7px;font-size:12px}@media(max-width:1320px){.pl-page .pl-grid{grid-template-columns:repeat(3,minmax(0,1fr))}}@media(max-width:1020px){.pl-page .pl-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:700px){.pl-page .pl-grid{grid-template-columns:1fr}.pl-page .pl-card .cg-thumb{height:146px}}' +
      '.pl-page .pl-grid{gap:16px}.pl-page .pl-card{position:relative;overflow:visible;border-color:#e2e8f0;border-radius:14px;box-shadow:0 2px 7px rgb(25 40 68 / 6%)}.pl-page .pl-visual{position:relative;height:174px;overflow:hidden;border-radius:13px 13px 0 0;background:linear-gradient(180deg,#f8fafc 0%,#eef2f7 100%)}.pl-page .pl-visual .cg-thumb{box-sizing:border-box;height:100%;padding:14px;border:0;background:transparent}.pl-page .pl-visual .pl-preview{border-radius:9px;box-shadow:0 8px 20px rgb(31 52 88 / 9%)}.pl-page .pl-visual .pl-preview-checkout-body{padding:9px;gap:8px}.pl-page .pl-visual .pl-preview-offer{gap:13px;padding:15px 17%}.pl-page .pl-visual .pl-preview-product{width:50px;height:50px;font-size:20px}.pl-preview-open{position:absolute;inset:0;display:grid;place-items:center;opacity:0;color:#fff;border:0;background:linear-gradient(180deg,rgb(24 37 60 / 8%),rgb(24 37 60 / 58%));font-size:13px;font-weight:700;cursor:pointer;transition:opacity .18s ease}.pl-visual:hover .pl-preview-open,.pl-preview-open:focus-visible{opacity:1}.pl-page .pl-card-body{padding:14px 14px 13px;gap:8px}.pl-page .pl-card-top{align-items:center}.pl-page .pl-type{font-size:10px;font-weight:700;letter-spacing:.7px}.pl-page .pl-status{padding:3px 7px;font-size:10px}.pl-page .pl-card h2{font-size:15px;line-height:1.25}.pl-page .pl-description{min-height:0;font-size:12px;line-height:1.45}.pl-usage{margin-top:2px;color:#71809a;font-size:11.5px;line-height:1.4}.pl-usage b{color:#43516a;font-weight:650}.pl-card-actions{display:flex;align-items:center;gap:8px;margin-top:3px}.pl-card-actions .btn{flex:1;justify-content:center;min-height:32px;font-size:12px;text-decoration:none}.pl-more{position:relative;z-index:3}.pl-more summary{display:grid;width:32px;height:32px;list-style:none;place-items:center;color:#52617a;border:1px solid #d7deea;border-radius:8px;background:#fff;font-size:16px;line-height:1;cursor:pointer}.pl-more summary::-webkit-details-marker{display:none}.pl-more[open] summary,.pl-more summary:hover{color:#2453d4;border-color:#aebff3;background:#f5f7ff}.pl-more-menu{position:absolute;right:0;bottom:39px;display:grid;min-width:112px;padding:5px;border:1px solid #dce3ee;border-radius:9px;background:#fff;box-shadow:0 10px 28px rgb(20 35 60 / 16%)}.pl-more-menu button{padding:8px 9px;color:#43516a;border:0;border-radius:6px;background:transparent;font:inherit;font-size:12px;text-align:left;cursor:pointer}.pl-more-menu button:hover{color:#2453d4;background:#f4f7ff}.pl-coming-soon{display:inline-flex;align-items:center;min-height:32px;color:#7a879c;font-size:12px}.pl-page .pl-facts,.pl-page .pl-metric,.pl-page .pl-actions{display:none}@media(max-width:1320px){.pl-page .pl-grid{grid-template-columns:repeat(3,minmax(0,1fr))}}@media(max-width:1020px){.pl-page .pl-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:700px){.pl-page .pl-grid{grid-template-columns:1fr}.pl-page .pl-visual{height:184px}}' +
    ...
    var libraryBreakpointStyle = document.createElement('style');
    libraryBreakpointStyle.textContent = '@media (min-width:1101px) and (max-width:1320px){.pl-page .pl-grid{grid-template-columns:repeat(4,minmax(0,1fr))}}';
    root.appendChild(libraryBreakpointStyle);
    var pageLibraryCompactStyle = document.createElement('style');
    pageLibraryCompactStyle.textContent = '.pl-page .pl-info{margin-bottom:16px}.pl-page .pl-filters,.pl-page .pl-count{padding-bottom:0}.pl-page .pl-preview-fields{gap:5px}.pl-page .pl-preview-fields span{border-color:transparent;background:#e3e8ee}.pl-preview-form-stack{display:grid;gap:4px}.pl-preview-payment-block{display:grid;gap:4px;padding:4px;border-radius:5px;background:#f4f6f9}.pl-preview-payment-block>i{display:block;width:28px;height:3px;border-radius:2px;background:#c5ceda}.pl-preview-card-fields{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:4px}.pl-preview-card-fields span:first-child{grid-column:1/-1}.pl-page .pl-preview-fields .pl-preview-pay{display:block;height:8px;border-radius:3px;background:var(--acc);margin-top:auto}.pl-page-preview-surface .pl-preview-fields .pl-preview-pay{height:13px}.pl-page .pl-preview-checkout aside{gap:5px}.pl-preview-summary-title{display:block;width:32px;height:3px;border-radius:2px;background:#c5ceda}.pl-page .pl-preview-summary-divider{display:block;height:1px;margin:1px 0;background:#e2e7ee}.pl-page .pl-preview-summary-discount{display:flex;height:9px;align-items:center;gap:3px;border:0;border-radius:3px;background:#edf3f8;padding:0 4px}.pl-page .pl-preview-summary-discount i{width:5px;height:5px;flex:none;border-radius:2px;background:#9fadbc}.pl-page .pl-preview-summary-discount span{display:block;width:54%;height:3px;flex:none;border-radius:2px;background:#c4d0dc}.pl-page .pl-preview-summary-totals{display:grid;gap:3px}.pl-page .pl-preview-summary-totals p{display:flex;margin:0;align-items:center;justify-content:space-between;gap:4px}.pl-page .pl-preview-summary-totals b{width:20px;height:3px;border-radius:2px;background:#cbd3de}.pl-page .pl-preview-summary-totals p b:first-child{width:24px;background:#d7dee7}.pl-page .pl-preview-summary-totals .pl-preview-summary-grandtotal{margin-top:1px;border-top:1px solid #e2e7ee;padding-top:3px}.pl-page .pl-preview-summary-totals .pl-preview-summary-grandtotal b{height:4px;background:#9ca9ba}.pl-page .pl-preview-summary-totals .pl-preview-summary-grandtotal b:first-child{background:#c3ccd8}.pl-page .pl-visual .pl-preview-nav b{display:block;width:28px;height:4px;border-radius:2px;background:#314157;color:transparent;font-size:0}.pl-page .pl-type{display:inline-flex;min-height:20px;align-items:center;border:1px solid transparent;border-radius:999px;padding:0 7px;font-size:10px;font-weight:700;letter-spacing:.25px;line-height:18px;text-transform:uppercase}.pl-page .pl-type-checkout{border-color:#d7e4ff;background:#eef4ff;color:#2f64d6}.pl-page .pl-type-thankyou{border-color:#ccebdd;background:#ecfaf2;color:#167a56}.pl-page .pl-type-upsell{border-color:#e3d8ff;background:#f5f1ff;color:#704fc2}.pl-page .pl-type-downsell{border-color:#f6ddbd;background:#fff5e9;color:#a65d15}.pl-page .pl-more summary{width:28px;height:28px;border-radius:7px;font-size:11px;letter-spacing:-1px}.pl-page .pl-more-menu{min-width:96px;bottom:35px}';
    root.appendChild(pageLibraryCompactStyle);
  ...
  // Category values are stable IDs. A product carries its complete path so a
  // merchant can deliberately target a parent category or any child category.
  var FLOW_CART_CATEGORY_TREE = [
    { value: 'health-wellness', label: 'Health & wellness', children: [
      { value: 'wellness', label: 'Wellness', children: [{ value: 'travel-wellness', label: 'Travel wellness' }, { value: 'daily-wellness', label: 'Daily wellness' }] },
      { value: 'supplements', label: 'Supplements', children: [{ value: 'sleep-support', label: 'Sleep support' }, { value: 'daily-nutrition', label: 'Daily nutrition' }] },
      { value: 'hydration', label: 'Hydration', children: [{ value: 'electrolytes', label: 'Electrolytes' }, { value: 'hydration-accessories', label: 'Hydration accessories' }] }
    ] },
    { value: 'bundles', label: 'Bundles', children: [{ value: 'starter-kits', label: 'Starter kits' }, { value: 'daily-routines', label: 'Daily routines' }, { value: 'sleep-routines', label: 'Sleep routines' }] },
    { value: 'accessories', label: 'Accessories', children: [{ value: 'travel-accessories', label: 'Travel accessories' }, { value: 'home-accessories', label: 'Home accessories' }] }
  ];
  var FLOW_CART_CATALOG = [
    { id: 'product-travel-wellness-kit', name: 'Travel wellness kit', handle: 'travel-wellness-kit', category: 'Travel wellness', categoryIds: ['health-wellness', 'wellness', 'travel-wellness'], status: 'Active', inventory: 218, price: 29.00, variants: [{ id: 'variant-travel-vanilla', name: 'Vanilla', sku: 'TWK-VAN-001', inventory: 110, price: 29.00 }, { id: 'variant-travel-citrus', name: 'Citrus', sku: 'TWK-CIT-002', inventory: 108, price: 29.00 }] },
    { id: 'product-sleep-support', name: 'Sleep support capsules', handle: 'sleep-support-capsules', category: 'Sleep support', categoryIds: ['health-wellness', 'supplements', 'sleep-support'], status: 'Active', inventory: 64, price: 24.00, variants: [{ id: 'variant-sleep-60', name: '60 capsules', sku: 'SLP-060', inventory: 46, price: 24.00 }, { id: 'variant-sleep-120', name: '120 capsules', sku: 'SLP-120', inventory: 18, price: 42.00 }] },
    { id: 'product-sleep-reset-bundle', name: 'Sleep Reset Bundle', handle: 'sleep-reset-bundle', category: 'Sleep routines', categoryIds: ['bundles', 'sleep-routines'], status: 'Active', inventory: 32, price: 48.00, variants: [{ id: 'variant-sleep-reset-blue', name: 'Blue', sku: 'SLEEP-SET-BLUE', inventory: 14, price: 48.00 }, { id: 'variant-sleep-reset-green', name: 'Green', sku: 'SLEEP-SET-GREEN', inventory: 18, price: 48.00 }] },
    { id: 'product-daily-essentials', name: 'Daily essentials bundle', handle: 'daily-essentials-bundle', category: 'Starter kits', categoryIds: ['bundles', 'starter-kits'], status: 'Active', inventory: 142, price: 36.00, variants: [{ id: 'variant-daily-standard', name: 'Standard bundle', sku: 'DEB-STD-01', inventory: 94, price: 36.00 }, { id: 'variant-daily-family', name: 'Family bundle', sku: 'DEB-FAM-02', inventory: 48, price: 54.00 }] },
    { id: 'product-hydration', name: 'Electrolyte hydration pack', handle: 'electrolyte-hydration-pack', category: 'Electrolytes', categoryIds: ['health-wellness', 'hydration', 'electrolytes'], status: 'Draft', inventory: 0, price: 18.00, variants: [{ id: 'variant-hydration-berry', name: 'Berry', sku: 'EHP-BER-12', inventory: 0, price: 18.00 }, { id: 'variant-hydration-lemon', name: 'Lemon', sku: 'EHP-LEM-12', inventory: 0, price: 18.00 }] },
    { id: 'product-daily-defense', name: 'Daily Defense multivitamin', handle: 'daily-defense-multivitamin', category: 'Daily nutrition', categoryIds: ['health-wellness', 'supplements', 'daily-nutrition'], status: 'Active', inventory: 186, price: 28.00, variants: [{ id: 'variant-defense-30', name: '30-day supply', sku: 'DDM-030', inventory: 102, price: 28.00 }, { id: 'variant-defense-90', name: '90-day supply', sku: 'DDM-090', inventory: 84, price: 72.00 }] },
    { id: 'product-magnesium-night', name: 'Magnesium Night Powder', handle: 'magnesium-night-powder', category: 'Sleep support', categoryIds: ['health-wellness', 'supplements', 'sleep-support'], status: 'Active', inventory: 92, price: 31.00, variants: [{ id: 'variant-magnesium-moon', name: 'Moon milk', sku: 'MNP-MON-01', inventory: 52, price: 31.00 }, { id: 'variant-magnesium-cocoa', name: 'Cocoa', sku: 'MNP-COC-01', inventory: 40, price: 31.00 }] },
    { id: 'product-focus-drops', name: 'Calm Focus Drops', handle: 'calm-focus-drops', category: 'Daily wellness', categoryIds: ['health-wellness', 'wellness', 'daily-wellness'], status: 'Active', inventory: 75, price: 22.00, variants: [{ id: 'variant-focus-30', name: '30 ml', sku: 'CFD-030', inventory: 75, price: 22.00 }] },
    { id: 'product-electrolyte-tubes', name: 'Electrolyte tubes', handle: 'electrolyte-tubes', category: 'Electrolytes', categoryIds: ['health-wellness', 'hydration', 'electrolytes'], status: 'Active', inventory: 310, price: 16.00, variants: [{ id: 'variant-tubes-citrus', name: 'Citrus', sku: 'ELT-CIT-08', inventory: 158, price: 16.00 }, { id: 'variant-tubes-berry', name: 'Berry', sku: 'ELT-BER-08', inventory: 152, price: 16.00 }] },
    { id: 'product-insulated-bottle', name: 'Insulated hydration bottle', handle: 'insulated-hydration-bottle', category: 'Hydration accessories', categoryIds: ['health-wellness', 'hydration', 'hydration-accessories'], status: 'Active', inventory: 57, price: 34.00, variants: [{ id: 'variant-bottle-sage', name: 'Sage', sku: 'IHB-SAG-01', inventory: 21, price: 34.00 }, { id: 'variant-bottle-sand', name: 'Sand', sku: 'IHB-SND-01', inventory: 36, price: 34.00 }] },
    { id: 'product-morning-routine', name: 'Morning Routine Bundle', handle: 'morning-routine-bundle', category: 'Daily routines', categoryIds: ['bundles', 'daily-routines'], status: 'Active', inventory: 48, price: 58.00, variants: [{ id: 'variant-morning-core', name: 'Core', sku: 'MRB-COR-01', inventory: 30, price: 58.00 }, { id: 'variant-morning-plus', name: 'Plus', sku: 'MRB-PLS-01', inventory: 18, price: 74.00 }] },
    { id: 'product-weekend-reset', name: 'Weekend Reset Kit', handle: 'weekend-reset-kit', category: 'Starter kits', categoryIds: ['bundles', 'starter-kits'], status: 'Active', inventory: 66, price: 44.00, variants: [{ id: 'variant-weekend-compact', name: 'Compact', sku: 'WRK-CMP-01', inventory: 39, price: 44.00 }, { id: 'variant-weekend-complete', name: 'Complete', sku: 'WRK-COM-01', inventory: 27, price: 62.00 }] },
    { id: 'product-sleep-routine', name: 'Evening Wind-down Set', handle: 'evening-wind-down-set', category: 'Sleep routines', categoryIds: ['bundles', 'sleep-routines'], status: 'Active', inventory: 41, price: 52.00, variants: [{ id: 'variant-wind-down-standard', name: 'Standard', sku: 'EWS-STD-01', inventory: 41, price: 52.00 }] },
    { id: 'product-travel-case', name: 'Travel supplement case', handle: 'travel-supplement-case', category: 'Travel accessories', categoryIds: ['accessories', 'travel-accessories'], status: 'Active', inventory: 120, price: 14.00, variants: [{ id: 'variant-case-lilac', name: 'Lilac', sku: 'TSC-LIL-01', inventory: 62, price: 14.00 }, { id: 'variant-case-charcoal', name: 'Charcoal', sku: 'TSC-CHR-01', inventory: 58, price: 14.00 }] },
    { id: 'product-pill-organizer', name: 'Weekly pill organizer', handle: 'weekly-pill-organizer', category: 'Home accessories', categoryIds: ['accessories', 'home-accessories'], status: 'Active', inventory: 84, price: 19.00, variants: [{ id: 'variant-organizer-sunday', name: 'Sunday start', sku: 'WPO-SUN-01', inventory: 42, price: 19.00 }, { id: 'variant-organizer-monday', name: 'Monday start', sku: 'WPO-MON-01', inventory: 42, price: 19.00 }] },
    { id: 'product-daily-protein', name: 'Daily Protein + Fiber', handle: 'daily-protein-fiber', category: 'Daily nutrition', categoryIds: ['health-wellness', 'supplements', 'daily-nutrition'], status: 'Active', inventory: 139, price: 39.00, variants: [{ id: 'variant-protein-vanilla', name: 'Vanilla', sku: 'DPF-VAN-01', inventory: 71, price: 39.00 }, { id: 'variant-protein-cacao', name: 'Cacao', sku: 'DPF-CAC-01', inventory: 68, price: 39.00 }] },
    { id: 'product-travel-calm', name: 'Travel Calm chewables', handle: 'travel-calm-chewables', category: 'Travel wellness', categoryIds: ['health-wellness', 'wellness', 'travel-wellness'], status: 'Draft', inventory: 27, price: 17.00, variants: [{ id: 'variant-travel-calm-mint', name: 'Mint', sku: 'TCC-MNT-01', inventory: 27, price: 17.00 }] },
    { id: 'product-gift-wellness', name: 'Wellness Gift Set', handle: 'wellness-gift-set', category: 'Starter kits', categoryIds: ['bundles', 'starter-kits'], status: 'Archived', inventory: 0, price: 68.00, variants: [{ id: 'variant-gift-wellness', name: 'Gift box', sku: 'WGS-GFT-01', inventory: 0, price: 68.00 }] },
    { id: 'product-home-tray', name: 'Bedside routine tray', handle: 'bedside-routine-tray', category: 'Home accessories', categoryIds: ['accessories', 'home-accessories'], status: 'Active', inventory: 36, price: 26.00, variants: [{ id: 'variant-tray-oak', name: 'Oak', sku: 'BRT-OAK-01', inventory: 20, price: 26.00 }, { id: 'variant-tray-white', name: 'White', sku: 'BRT-WHT-01', inventory: 16, price: 26.00 }] },
    { id: 'product-probiotic-daily', name: 'Daily probiotic capsules', handle: 'daily-probiotic-capsules', category: 'Daily nutrition', categoryIds: ['health-wellness', 'supplements', 'daily-nutrition'], status: 'Active', inventory: 97, price: 27.00, variants: [{ id: 'variant-probiotic-30', name: '30 capsules', sku: 'DPC-030', inventory: 54, price: 27.00 }, { id: 'variant-probiotic-60', name: '60 capsules', sku: 'DPC-060', inventory: 43, price: 48.00 }] },
    { id: 'product-hydration-sachets', name: 'Travel hydration sachets', handle: 'travel-hydration-sachets', category: 'Electrolytes', categoryIds: ['health-wellness', 'hydration', 'electrolytes'], status: 'Active', inventory: 208, price: 21.00, variants: [{ id: 'variant-sachets-grapefruit', name: 'Grapefruit', sku: 'THS-GRP-10', inventory: 104, price: 21.00 }, { id: 'variant-sachets-peach', name: 'Peach', sku: 'THS-PCH-10', inventory: 104, price: 21.00 }] },
    { id: 'product-storage-jars', name: 'Stacking supplement jars', handle: 'stacking-supplement-jars', category: 'Home accessories', categoryIds: ['accessories', 'home-accessories'], status: 'Active', inventory: 45, price: 18.00, variants: [{ id: 'variant-jars-small', name: 'Small set', sku: 'SSJ-SML-01', inventory: 27, price: 18.00 }, { id: 'variant-jars-large', name: 'Large set', sku: 'SSJ-LRG-01', inventory: 18, price: 24.00 }] }
  ];
  var FLOW_LIST_SEED = [
  ...
  function bcRemoveLegacyOrderBumpData(flows) {
    var legacyNames = {
      'Order bump': 'Order value booster',
      'Order bump flow': 'Order value booster',
      'Order bump offer': 'Order value booster',
      '订单加购': 'Order value booster',
      '订单加购流程': 'Order value booster',
      '结账加购': 'Order value booster'
    };
    var changed = false;
    (flows || []).forEach(function (flow) {
      if (!flow) return;
      if (legacyNames[flow.name]) { flow.name = legacyNames[flow.name]; changed = true; }
      (flow.steps || []).forEach(function (step) {
        if (!Array.isArray(step.blocks)) return;
        var kept = step.blocks.filter(function (block) { return block && block.type !== 'orderBump'; });
        if (kept.length !== step.blocks.length) { step.blocks = kept; changed = true; }
      });
    });
    return changed;
  }
  function bcApplyOrderBumpCleanup(flows) {
    try {
      if (localStorage.getItem(FLOW_ORDER_BUMP_CLEANUP_KEY)) return flows;
      if (bcRemoveLegacyOrderBumpData(flows)) bcFlowListSave(flows);
      localStorage.setItem(FLOW_ORDER_BUMP_CLEANUP_KEY, '1');
    } catch (e) {}
    return flows;
  }
  ...
  function openFlowEntryUnifiedItemPicker(field, selectedValues, onApply) {
    ...
    renderList = function () {
      ...
      var matchesCategory = function (product) {
        var categoryIds = product.categoryIds || [product.category];
        return category === 'all' || categoryIds.indexOf(category) !== -1;
      };
      var groups = FLOW_CART_CATALOG.filter(function (product) {
        return matchesText(product) && matchesCategory(product) && (statusFilter === 'all' || product.status === statusFilter) && matchesPrice(product);
      });
    ...
    var renderAppliedFilters = function () {
      ...
      if (category !== 'all') filters.push({ key: 'category', label: t('Category') + ': ' + pickerCategoryLabel(category) });
    ...
    var categoryTree = FLOW_CART_CATEGORY_TREE;
    ...
    var openCategoryMenu = function (trigger) {
      ...
      var renderCategoryMenu = function () {
        ...
        var heading = path.length ? '<button type="button" class="flow-entity-category-head" data-flow-entity-category-back aria-label="' + esc(t('Back')) + '"><span aria-hidden="true">‹</span><span>' + esc(path[path.length - 1].label) + '</span></button>' : '';
        ...
        menu.innerHTML = heading + all + nodes.map(function (node, index) {
          ...
          var count = FLOW_CART_CATALOG.filter(function (product) { return (product.categoryIds || [product.category]).indexOf(node.value) !== -1; }).length;
          return '<div class="flow-entity-category-row"><button type="button" class="flow-entity-category-option" data-flow-entity-category-pick="' + esc(node.value) + '"><span>' + esc(node.label) + '</span><small>' + count + '</small></button>' + (hasChildren ? '<button type="button" class="flow-entity-category-drill" data-flow-entity-category-enter="' + index + '" aria-label="' + esc(t('Show subcategories for ') + node.label) + '">›</button>' : '') + '</div>';
        }).join('');
        var back = menu.querySelector('[data-flow-entity-category-back]');
        if (back) back.onclick = function () { path.pop(); renderCategoryMenu(); };
        menu.querySelectorAll('[data-flow-entity-category-pick]').forEach(function (button) {
          button.onclick = function () {
            var value = button.getAttribute('data-flow-entity-category-pick');
            pickerSelect('[data-flow-entity-category]', value);
            pickerSetLabel('category', value === 'all' ? t('Category') : pickerCategoryLabel(value), value !== 'all');
            pickerFilterDismiss(); pickerPage = 1; renderList();
          };
        });
        menu.querySelectorAll('[data-flow-entity-category-enter]').forEach(function (button) {
          button.onclick = function () {
            var node = nodes[Number(button.getAttribute('data-flow-entity-category-enter'))];
            if (!node || !node.children || !node.children.length) return;
            path.push(node); renderCategoryMenu();
          };
        });
      };
      renderCategoryMenu();
  ...
  function renderFlowList(filter) {
    ...
    var flowListEye = '<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M2 12s3.5-7 10-7 10 7 10 7-3.5 7-10 7-10-7-10-7Z"/><circle cx="12" cy="12" r="3"/></svg>';
    tableRows = pageFlows.map(function (flow) {
      var detailLabel = t('Open') + ': ' + t(flow.name);
      return '<tr class="fl-row" data-flow-row="' + esc(flow.id) + '" tabindex="0" role="link" aria-label="' + esc(detailLabel) + '">' +
        '<td><span class="fl-name">' + esc(t(flow.name)) + '</span><div class="fl-sub">' + esc(flowEntrySummary(flow)) + '</div><div class="fl-meta">' + t('Priority') + ' ' + esc(flow.priority) + ' &middot; <span class="fl-path">' + esc(flowListPagePath(flow)) + '</span></div></td>' +
        '<td>' + flowPill(flow.status) + '</td><td><b>' + esc(flow.traffic) + '</b><small>' + t('of checkout traffic') + '</small></td><td><b>' + esc(flow.conversion) + '</b><small>' + t('conversion') + '</small></td><td><b>' + esc(flow.aov) + '</b><small>' + t('average order value') + '</small></td>' +
        '<td class="fl-view-cell"><button type="button" class="fl-view" data-flow-view="' + esc(flow.id) + '" title="' + esc(detailLabel) + '" aria-label="' + esc(detailLabel) + '">' + flowListEye + '</button></td></tr>';
    }).join('');
    root.innerHTML = wrap(GSTYLE + FSTYLE + '<style>' +
      '.fl-table tbody tr.fl-row{cursor:pointer}.fl-table tbody tr.fl-row:hover td{background:#fbfdff}.fl-table tbody tr.fl-row:focus-visible{outline:2px solid var(--brand);outline-offset:-2px}.fl-view-cell,.fl-action-head{width:52px;text-align:center!important;white-space:nowrap}.fl-view{display:inline-grid;width:30px;height:30px;place-items:center;border:0;border-radius:6px;background:transparent;color:var(--ink-muted);cursor:pointer}.fl-view:hover,.fl-view:focus-visible{background:var(--panel);color:var(--brand);outline:none}.fl-view svg{display:block}' +
    ...
    root.querySelectorAll('[data-flow-row]').forEach(function (row) {
      var open = function () { location.hash = '#/flows/' + row.getAttribute('data-flow-row'); };
      row.onclick = function (event) { if (event.target.closest('[data-flow-view]')) return; open(); };
      row.onkeydown = function (event) { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); open(); } };
    });
    root.querySelectorAll('[data-flow-view]').forEach(function (button) { button.onclick = function (event) { event.stopPropagation(); location.hash = '#/flows/' + button.getAttribute('data-flow-view'); }; });
  ...
  function openCreateFlowWizard() {
    ...
    var render = function () {
      ...
      if (step === 'goal') {
        ...
        body = '<label class="fcw-field">' + t('Purchase flow name') + '<input class="input" id="fcw-name" value="' + esc(flowName) + '" maxlength="80" autofocus aria-describedby="fcw-name-error"><span class="fcw-name-error" id="fcw-name-error" data-fcw-name-error role="alert"></span></label><div class="fcw-journey"><small>' + t('Starting journey') + '</small><strong>' + t(selectedGoal.label) + '</strong><span class="fcw-path">' + t(selectedGoal.path) + '</span></div><section class="fcw-conditions"><header><div><strong>' + t('Who enters this purchase flow? (AND)') + '</strong><p>' + t('Add customer attributes, storefront, or cart conditions directly. Every condition must match.') + '</p></div><span>' + t('No match → Shopify Checkout') + '</span></header><div class="fcw-condition-list">' + conditions + '</div><button type="button" class="fcw-condition-add" data-fcw-condition-add>+ ' + t('Add condition') + '</button><p class="fcw-condition-help">' + t('Customer order history and tags apply only to recognized Shopify customers.') + '</p></section><p class="fcw-msg" data-fcw-msg></p>';
      }
      var nameErrorStyle = '.fcw-field .input.is-error{border-color:var(--err)!important;box-shadow:0 0 0 2px rgba(255,77,79,.13)!important}.fcw-name-error{display:none;color:var(--err);font-size:12px;font-weight:400;line-height:1.35}.fcw-name-error.is-visible{display:block}';
      modal.innerHTML = XSTYLE + FSTYLE + '<style>' +
        nameErrorStyle +
      ...
      modal.querySelector('[data-fcw-next]').onclick = function () {
        ...
        if (!flowName) { if (input) { input.classList.add('is-error'); input.setAttribute('aria-invalid', 'true'); var nameError = modal.querySelector('[data-fcw-name-error]'); if (nameError) { nameError.textContent = t('Enter a purchase flow name.'); nameError.classList.add('is-visible'); } input.focus(); } return; }
...
'.fj-stage-checkout .fj-stage-nodes{display:flex;flex-direction:column;gap:9px}.fj-checkout-branches{position:relative;display:grid;gap:12px;padding:0 20px}.fj-checkout-branches:before,.fj-checkout-branches:after{position:absolute;top:28px;bottom:28px;width:1px;border:0;background:#b8c5d3;content:""}.fj-checkout-branches:before{left:6px}.fj-checkout-branches:after{right:6px}.fj-checkout-branches .fj-node{position:relative}.fj-checkout-branches .fj-node:before,.fj-checkout-branches .fj-node:after{position:absolute;top:50%;width:14px;height:1px;border:0;border-radius:0;background:#b8c5d3;content:""}.fj-checkout-branches .fj-node:before{left:-14px}.fj-checkout-branches .fj-node:after{right:-14px;left:auto}.fj-offer-branch{display:flex;align-items:center;gap:5px;height:30px;margin:0;padding:0 12px}.fj-offer-branch:before,.fj-offer-branch:after{display:none}.fj-decline-marker{position:static;transform:none;padding:2px 7px;color:#9f6512;background:#fff8eb;border:1px solid #efd6a1;border-radius:999px;font-size:9px;font-weight:700}.fj-branch-arrow{position:static;transform:none;padding:0;color:transparent;background:transparent;font-size:0;font-weight:700;line-height:1}.fj-branch-arrow:after{color:#b97917;font-size:14px;content:"\\2193"}.fj-outcome.complete b{color:#56677a}.fj-outcomes-shared{margin-top:8px}' +
  ...
  function renderFlowJourney(flow, st, dirty) {
    ...
    root.querySelectorAll('[data-flow-delete-checkout]').forEach(function (button) { button.onclick = function () { fnDeletePage(button.getAttribute('data-flow-delete-checkout')); }; });
  ...
  function flowCanvasNode(nd) {
    ...
    if (n.ab) {
      ...
      body = '<div class="fn-ab"><div class="fn-ab-h">A/B · ' + esc(bcTplName(n.type, n.tpl)) + ' vs ' + esc(bcTplName(n.type, n.ab.b)) + (byUser ? ' <span class="fn-ab-mode">' + t('by user type') + '</span>' : '') + '</div>' +
        ...
        '<div class="fn-ab-foot"><a href="' + bcEditHash(n.type, n.tpl, '#/flows/' + activeFlowId) + '">' + t('Edit A') + '</a> · <a href="' + bcEditHash(n.type, n.ab.b, '#/flows/' + activeFlowId) + '">' + t('Edit B') + '</a> · <a href="#" data-rmab="' + n.id + '">' + t('Remove A/B') + '</a></div></div>';
    } else {
...
'<button class="btn btn-default" data-z="out" title="Zoom out">−</button><span class="fc-zval" id="fc-z">100%</span><button class="btn btn-default" data-z="in" title="Zoom in">+</button><button class="btn btn-default" data-z="fit">' + t('Fit') + '</button>' +
  ...
  function openCheckoutTrafficEditor(fromId, s0, buf) {
    ...
    var branchMarkup = function (branch, index) {
      ...
      return '<div class="ct-route"><span class="ct-route-icon">' + (index + 1) + '</span><div class="ct-route-copy"><strong>' + t('Checkout') + ' ' + (index + 1) + '</strong><small>' + detail + '</small></div><label class="ct-percent"><input type="number" min="0" max="100" step="1" value="' + esc(String(weights[index])) + '" data-ct-weight="' + index + '"><span>%</span></label></div>';
    };
    function render() {
      routesEl.innerHTML = '<section class="ct-platform"><div class="ct-platform-head"><div class="ct-platform-copy"><b>BestCheckout</b><small>' + t('Checkout pages') + '</small></div><span class="ct-platform-note">' + t('Set one default Checkout or split the same audience across multiple pages.') + '</span></div>' + (buf.map(branchMarkup).join('') || '<div class="rb-empty">' + t('No BestCheckout Checkout page yet.') + '</div>') + '</section>';
  ...
  function openOfferProductPicker(selectedId, onPick) {
    ...
    if (window.UI && typeof window.UI.productPicker === 'function') {
      window.UI.productPicker({
        ...
        categories: [{ value: 'add-ons', label: 'Products' }, { value: 'bundles', label: 'Bundles' }],
  ...
  function fnDeletePage(id) {
    ...
    if (node && node.type === 'checkout') {
      var checkoutBranches = fnCheckoutTrafficEdges(s);
      checkoutBranches.forEach(function (branch, index) {
        branch.rule = branch.rule || { type: 'expression', conditions: [] };
        branch.rule.conditions = (branch.rule.conditions || []).filter(function (condition) { return condition.field !== 'random'; });
        branch.rule.conditions.push({ field: 'random', op: 'pct', value: index === 0 ? 100 : 0 });
        branch.rule.fallback = false;
      });
    }
  ...
  function openFunnelAB(id) {
    ...
    if (!opts) { toast(t('This page has only one template — open 装修, then “Save as template” to make variant B, and A/B-test it.')); location.hash = bcEditHash(type, cur, '#/flows/' + activeFlowId); return; }
    var m = document.createElement('div'); m.className = 'xp-modal';
  ...
  const ZH = {
    ...
    'multi-MID routing + cascade': '多 MID 路由 + 级联', 'post-purchase Upsell + Downsell': '购后 Upsell + Downsell', 'cycle-over-cycle': '逐周期', 'RDR + Ethoca + 3DS': 'RDR + Ethoca + 3DS', 'routed via BestCheckout': '经 BestCheckout 路由', 'cascade + recycle saves': '级联 + 回收救回',
    ...
    'The funnel': '转化漏斗', 'Checkout': '结账', 'Upsell': '追加', 'Downsell': '降级', 'Thank you': '致谢页',
    'Single-page checkout': '单页结账', 'One-click, no re-enter card': '一键，无需重输卡', 'On decline of the upsell': '追加被拒时', 'Write order back to Shopify': '订单写回 Shopify',
    ...
    'fast single-page checkout': '极速单页结账', 'post-purchase Upsell + Downsell': '购后 Upsell + Downsell', 'one-click, no re-enter card': '一键，无需重输卡', 'captured, written back to Shopify': '已捕获并回写 Shopify', 'through BestCheckout': '经 BestCheckout', '3DS on high-risk orders': '高风险订单走 3DS',
  ...
  Object.assign(ZH, {
    ...
    'Upsell': '加购推荐', 'Sleep Bundle offer': '睡眠套装优惠', 'buyers accepted after payment': '买家在付款后接受', 'Downsell': '降价替代推荐', 'Half-size alternative': '半规格替代优惠', 'recovered after the first offer was declined': '首次优惠被拒后挽回', 'Accept rate': '接受率', 'Added sales': '带来销售额',
  ...
  function renderPerformance() {
    ...
    var offerSales = performanceRange === '7d' ? ['$8,642', '$2,910'] : performanceRange === '30d' ? ['$35,120', '$11,240'] : ['$92,750', '$29,380'];
    var offers = [
